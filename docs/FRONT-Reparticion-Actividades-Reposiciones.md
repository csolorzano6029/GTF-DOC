...
- **Orden (según el video):** primero _administración_, luego _revisión_ `[00:48:59–00:50:10]`. Ambos van en este mismo proyecto.
- **Fecha:** 2026-07-06.
...
| A-3 | Editar cabecera + incrustar detalle | `POST /replenishments/{id}`                                                                                                                | Solo editable en `PENDING`; usa cabecera de fondo y visor de detalle compartidos.        |
| A-4 | Formularios por tipo de documento   | `POST /replenishments/{id}/details`, `POST /replenishments/{id}/details/{detailId}`, `POST /replenishments/{id}/details/{detailId}/delete` | Factura, factura electrónica, nota de venta, recibo manual (oculta IVA donde no aplica). |
...
| B-3 | Ajustar valor aprobado                        | `POST /replenishment-reviews/{id}/details/{detailId}/approve`          | Edita aprobado + observación; avisa que se enviará mail al local.                       |
