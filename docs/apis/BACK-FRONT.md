...
| POST /replenishment-management/validate-new | Implementado | Ejecutar precheck de flujo Nuevo (cheque emitido, pendiente por local, carga inicial de fondos y tipo de transaccion)                                             |
...
| Administracion de Reposiciones | Reposiciones > Administracion | Precheck de nuevo       | Validar precondiciones       | POST /replenishment-management/validate-new                                     | Implementado (P0 - precheck)            | Validar cheque emitido, pendiente por local, carga inicial y tipo de transaccion antes de crear           |
...
| Validacion        | `POST /replenishments/details/validate-add`                           | `workAreaCode`, `transactionCode`, `detail` (objeto detalle), `currentRequestedTotal` (opcional)                       | `workAreaCode` y `detail` obligatorios; `transactionCode` opcional (si no llega se asume `1`); `currentRequestedTotal` opcional                                                                                                                         | Misma respuesta compacta: banderas en `data` y errores en `BaseResponseVo.errors` (raiz), con `message` generico para errores de negocio                                                                                                                                                                                                                                 | Endpoint alterno para validar filas temporales cuando aun no existe `replenishmentId`                                                             |
...
| `POST /replenishments/details/validate-add`                           | `workAreaCode`, `detail` (body). Dentro de `detail`: `billingConceptSequence`, `documentType`, `documentDate`, `requestedValue`, `vatValue`; `transactionCode` opcional (`1` por defecto), `currentRequestedTotal` opcional |
| `POST /replenishment-management/{replenishmentId}/details`            | `replenishmentId` (path), `billingConceptSequence`, `documentType`, `documentDate`, `requestedValue`, `vatValue`                                                                                                            |
