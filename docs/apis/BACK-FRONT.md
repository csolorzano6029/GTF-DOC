...
| --------------------------------- | ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET /replenishments/current-fund  | Parcial      | Completar alertas operativas por porcentaje y reglas de reposicion de fondo (las validaciones de pendiente/cheque emitido migraron al precheck de `validate-new`) |
| POST /replenishments/validate-new | Implementado | Ejecutar precheck de flujo Nuevo (cheque emitido, pendiente por local, carga inicial de fondos y tipo de transaccion)                                             |
...
| Administracion de Reposiciones | Reposiciones > Administracion | Precheck de nuevo       | Validar precondiciones  | POST /replenishments/validate-new                   | Implementado (P0 - precheck)           | Validar cheque emitido, pendiente por local, carga inicial y tipo de transaccion antes de crear |
...
| Administracion de Reposiciones | Reposiciones > Administracion | Tabla de detalle        | Guardar varias filas    | POST /replenishments/{id}/details                   | Implementado (P0 - batch)              | Crear multiples lineas en una sola solicitud                                                    |
...
| Administracion de Reposiciones | Reposiciones > Administracion | Flujo de envio          | Enviar reposicion       | POST /replenishments/{id}/send                      | Implementado (P0 - corte 6)            | Cambiar estado y ejecutar validaciones de envio                                                 |
