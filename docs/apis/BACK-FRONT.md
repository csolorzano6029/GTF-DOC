...
| ------------------------------------------ | ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| GET /replenishment-management/current-fund | Parcial      | Completar alertas operativas por porcentaje y reglas de reposicion de fondo (las validaciones de pendiente/cheque emitido migraron al precheck de `validate-new`)           |
| GET /replenishment-management/validate-new | Implementado | Ejecutar precheck de flujo Nuevo (cheque emitido, pendiente por local, carga inicial de fondos y tipo de transaccion)                                                       |
| POST /replenishments/filter                | Parcial      | Forzar filtro por area de trabajo del usuario, mejorar soporte de filtros de fecha/estado como en legacy y mantener orden descendente por `createdDate` (recientes primero) |
| GET /replenishments/{id}                   | Parcial      | Incluir detalle completo de documentos y contexto operativo para edicion/revision                                                                                           |
| POST /replenishment-management             | Parcial      | Aplicar validaciones de detalle legacy (IVA, duplicados, monto maximo, reglas por transaccion, disponibilidad de fondo)                                                     |
| POST /replenishment-management/{id}        | Parcial      | Actualizar cabecera en estado pendiente (el detalle se actualiza por endpoints `/details`)                                                                                  |
...
| ------------------------------ | ----------------------------- | ----------------------- | ---------------------------- | ------------------------------------------------------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Administracion de Reposiciones | Reposiciones > Administracion | Lista de reposiciones   | Buscar                       | POST /replenishments/filter                                                     | Implementado (parcial)                  | Cargar grilla con filtros y paginacion                                                                    |
...
| Administracion de Reposiciones | Reposiciones > Administracion | Selector de concepto    | Abrir popup concepto         | GET /replenishments/billing-concepts                                            | Implementado (P0 - corte 7)             | Cargar conceptos por area/transaccion/tipo documento, agrupados por categoria                             |
| Administracion de Reposiciones | Reposiciones > Administracion | Selector de unidad      | Cargar unidades por concepto | GET /replenishments/billing-concepts/{billingConceptSequence}/measurement-units | Implementado (P0 - huella por concepto) | Cargar unidades permitidas para conceptos de huella de carbono                                            |
| Administracion de Reposiciones | Reposiciones > Administracion | Cabecera de fondo       | Refrescar cabecera           | GET /replenishment-management/current-fund?workAreaCode={id}                    | Implementado (parcial)                  | Mostrar fondo, saldo, alertas y estado actual                                                             |
| Administracion de Reposiciones | Reposiciones > Administracion | Precheck de nuevo       | Validar precondiciones       | GET /replenishment-management/validate-new                                      | Implementado (P0 - precheck)            | Validar cheque emitido, pendiente por local, carga inicial y tipo de transaccion antes de crear           |
| Administracion de Reposiciones | Reposiciones > Administracion | Formulario cabecera     | Nueva reposicion             | POST /replenishment-management                                                  | Implementado (parcial)                  | Crear solicitud en estado pendiente                                                                       |
| Administracion de Reposiciones | Reposiciones > Administracion | Formulario cabecera     | Guardar cambios              | POST /replenishment-management/{replenishmentId}                                | Implementado (parcial)                  | Editar cabecera en estado pendiente                                                                       |
...
| Administracion de Reposiciones | Reposiciones > Administracion | Tabla de detalle        | Cargar detalle               | GET /replenishments/{id}/details                                                | Implementado (P0 - corte 5)             | Consultar lineas de detalle por reposicion (ordenadas por `documentDate` ascendente)                      |
| Administracion de Reposiciones | Reposiciones > Administracion | Tabla de detalle        | Agregar fila                 | POST /replenishment-management/{id}/details                                     | Implementado (P0 - corte 3)             | Crear linea de detalle                                                                                    |
| Administracion de Reposiciones | Reposiciones > Administracion | Tabla de detalle        | Guardar varias filas         | POST /replenishment-management/{id}/details                                     | Implementado (P0 - batch)               | Crear multiples lineas en una sola solicitud                                                              |
| Administracion de Reposiciones | Reposiciones > Administracion | Tabla de detalle        | Editar fila                  | POST /replenishment-management/{id}/details/{detailId}                          | Implementado (P0 - corte 4)             | Actualizar linea de detalle                                                                               |
| Administracion de Reposiciones | Reposiciones > Administracion | Tabla de detalle        | Eliminar fila                | POST /replenishment-management/{id}/details/{detailId}/delete                   | Pendiente P0                            | Eliminar linea y recalcular totales                                                                       |
...
| Administracion de Reposiciones | Reposiciones > Administracion | Flujo de envio          | Enviar reposicion            | POST /replenishment-management/{id}/send                                        | Implementado (P0 - corte 6)             | Cambiar estado y ejecutar validaciones de envio (body opcional con responsable de cheque por id o cedula) |
| Administracion de Reposiciones | Reposiciones > Administracion | Flujo de envio          | Seleccionar responsable      | GET /replenishments/responsibles                                                | Implementado (P1 - corte responsibles)  | Cargar responsables para pago por ventanilla                                                              |
| Administracion de Reposiciones | Reposiciones > Administracion | Acciones de solicitud   | Anular                       | POST /replenishment-management/{id}/cancel                                      | Pendiente (definir Admin vs Revision)   | Anulacion con observacion                                                                                 |
| Administracion de Reposiciones | Reposiciones > Administracion | Acciones de solicitud   | Imprimir                     | GET /replenishment-management/{id}/print                                        | Implementado (P1 - corte print)         | Generar y descargar comprobante PDF de reposicion enviada                                                 |
...
Esta subseccion aterriza el formulario de cabecera de Administracion contra los endpoints `POST /replenishment-management` y `POST /replenishment-management/{replenishmentId}`.
...
Esta subseccion aterriza el formulario de detalle de Administracion contra los endpoints `POST /replenishment-management/{replenishmentId}/details` y `POST /replenishment-management/{replenishmentId}/details/{detailId}`.
...
- Para create batch, todas las lineas del lote se validan antes de persistir; si una falla, no se persiste ninguna.
...
- En create batch tambien se valida duplicado interno dentro del mismo request (`taxId + documentNumber`).
- En update, si cambia el identificador documental (`taxId + documentNumber`) se vuelve a validar duplicidad.
- Luego de crear o editar detalle, backend recalcula totales de cabecera (`requestedTotal`, `vatTotal`, `requestedSubtotal`). En batch el recálculo se ejecuta una sola vez al final del lote.
...
### 4.1.1.4 Mapeo puntual de cabecera de fondo (`GET /replenishment-management/current-fund`)
...
| Tabla detalle        | Fecha documento               | `documentDate`                                                       | Formato actual: epoch numerico (milisegundos). Lista ordenada ascendente por `documentDate` (mas antiguos primero). |
...
| Consulta          | `GET /replenishments/{replenishmentId}/details`                       | Path `replenishmentId`                                                                                                 | `replenishmentId` obligatorio                                                                                                                                                                                                                           | Lista `ReplenishmentDetailVo` enriquecida (`saleReceipt`, `measurementTypeCode`, `measurementValueCode`, `measurementUnitName`, `fileName`, `billingConceptDescription`, `documentTypeName`), ordenada por `documentDate` ascendente (documentos mas antiguos primero)                                                                                                                                                               | `documentTypeName` puede llegar `null` si no existe representacion en catalogo                                                                    |
| Mutacion cabecera | `POST /replenishment-management`                                      | En legacy se digita cedula (`personaResDTO.numeroDocumento`) + observacion + tipo.                                     | Requiere `workAreaCode`, `observation` y (`responsiblePersonId` o `responsiblePersonDocument`); `beneficiaryPersonId` es obligatorio cuando `transactionCode = "2"`; el resto es opcional.                                                              | `ReplenishmentVo` creado con estado pendiente y enriquecimiento (`workAreaName`, `responsiblePersonDocument`)                                                                                                                                                                                                                                                                                                                        | Sin brecha critica en identificacion de responsable para create                                                                                   |
| Mutacion cabecera | `POST /replenishment-management/{replenishmentId}`                    | Cabecera editable                                                                                                      | `replenishmentId` en path + body parcial permitido; backend conserva valores actuales no enviados y vuelve a validar reglas de cabecera.                                                                                                                | `ReplenishmentVo` actualizado con estado pendiente                                                                                                                                                                                                                                                                                                                                                                                   | Sin brecha critica para update de cabecera en este punto                                                                                          |
...
| Mutacion detalle  | `POST /replenishment-management/{replenishmentId}/details`            | Legacy arma fila con documento, ruc, no documento, fecha, concepto, cantidad, unidad, archivo, observacion, iva, total | Requiere: `billingConceptSequence`, `documentType`, `documentDate`, `requestedValue`, `vatValue`; opcional: `taxId`, `documentNumber`, `observation`, `accessKey`, `quantity`, `saleReceipt`, `measurementTypeCode`, `measurementValueCode`, `fileName` | `ReplenishmentDetailVo` con `detailId`, `replenishmentId` y campos del request                                                                                                                                                                                                                                                                                                                                                       | La descripcion legible (`measurementUnitName`, `billingConceptDescription`, `documentTypeName`) se confirma en `GET /replenishments/{id}/details` |
| Validacion        | `POST /replenishments/{replenishmentId}/details/validate-add`         | `replenishmentId` (path), `detail` (objeto detalle), `currentRequestedTotal` (opcional)                                | `replenishmentId` obligatorio en path; `detail` obligatorio en body; `currentRequestedTotal` opcional (si no llega, backend toma el total persistido)                                                                                                   | En `data`: `valid`, `sendAlert`, `blockedByAvailableBalance`, `blockedByUsagePercentage`; en raiz (`BaseResponseVo.errors`): `errors[]` cuando aplica. `message` se mantiene generico para errores de negocio                                                                                                                                                                                                                        | Nuevo endpoint para prevalidar reglas de negocio al agregar fila sin persistir                                                                    |
| Validacion        | `POST /replenishment-management/details/validate-add`                 | `transactionCode`, `detail` (objeto detalle), `currentRequestedTotal` (opcional)                                       | `detail` obligatorio; `transactionCode` opcional (si no llega se asume `1`); `currentRequestedTotal` opcional. En `detail` es obligatorio `observation`                                                                                                 | Misma respuesta compacta: banderas en `data` y errores en `BaseResponseVo.errors` (raiz), con `message` generico para errores de negocio                                                                                                                                                                                                                                                                                             | Endpoint alterno para validar filas temporales cuando aun no existe `replenishmentId`                                                             |
| Mutacion detalle  | `POST /replenishment-management/{replenishmentId}/details/{detailId}` | Mismos campos de detalle editable                                                                                      | `replenishmentId`, `detailId` path + body `ReplenishmentDetailVo` (mismas reglas de requeridos + opcionales de detalle)                                                                                                                                 | `ReplenishmentDetailVo` actualizado                                                                                                                                                                                                                                                                                                                                                                                                  | Sin gap funcional critico para edicion de campos base                                                                                             |

### 4.1.3 Campos obligatorios actuales por endpoint (sin ambiguedad)

| Endpoint                                                              | Campos obligatorios                                                                                                                                                                                                        |
| --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /replenishment-management/current-fund`                          | `workAreaCode` (query)                                                                                                                                                                                                     |
| `POST /replenishments/filter`                                         | Ninguno en body (si no llegan filtros, lista por compania)                                                                                                                                                                 |
| `GET /replenishments/{replenishmentId}`                               | `replenishmentId` (path)                                                                                                                                                                                                   |
| `GET /replenishments/{replenishmentId}/details`                       | `replenishmentId` (path)                                                                                                                                                                                                   |
| `POST /replenishment-management`                                      | `workAreaCode`, `observation`, (`responsiblePersonId` o `responsiblePersonDocument`); `beneficiaryPersonId` cuando `transactionCode = "2"`                                                                                 |
| `POST /replenishment-management/{replenishmentId}`                    | `replenishmentId` (path). El body puede ser parcial; se revalidan reglas de cabecera al consolidar con valores actuales.                                                                                                   |
| `POST /replenishments/validate-vat`                                   | `documentTotal`, `vatValue`                                                                                                                                                                                                |
| `POST /replenishments/validate-duplicate`                             | `taxId`, `documentNumber`                                                                                                                                                                                                  |
| `POST /replenishments/{replenishmentId}/details/validate-add`         | `replenishmentId` (path), `detail` (body). Dentro de `detail`: `billingConceptSequence`, `documentType`, `documentDate`, `requestedValue`, `vatValue`; `currentRequestedTotal` opcional                                    |
| `POST /replenishment-management/details/validate-add`                 | `detail` (body). Dentro de `detail`: `billingConceptSequence`, `documentType`, `documentDate`, `requestedValue`, `vatValue`, `observation`; `transactionCode` opcional (`1` por defecto), `currentRequestedTotal` opcional |
| `POST /replenishment-management/{replenishmentId}/details`            | `replenishmentId` (path), `billingConceptSequence`, `documentType`, `documentDate`, `requestedValue`, `vatValue`, `observation`                                                                                            |
| `POST /replenishment-management/{replenishmentId}/details/{detailId}` | `replenishmentId` (path), `detailId` (path), `billingConceptSequence`, `documentType`, `documentDate`, `requestedValue`, `vatValue`                                                                                        |
...
3. Para detalle, frontend debe usar `saleReceipt`, `measurementTypeCode`, `measurementValueCode` y `fileName` en altas/ediciones; para visualizacion legible usar `measurementUnitName` y `billingConceptDescription` desde `GET /replenishments/{id}/details`. Antes de agregar una fila en UI temporal, llamar `POST /replenishments/{id}/details/validate-add` cuando ya exista cabecera, o `POST /replenishment-management/details/validate-add` cuando la cabecera aun no se haya persistido; si el usuario registra varias filas y confirma guardado, enviar todo el lote en una sola solicitud a `POST /replenishment-management/{id}/details`. En validaciones fallidas de negocio, consumir `errors[]` en la raiz del `BaseResponseVo` para mostrar el detalle y no depender del texto de `message`.
